...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "importprivkey <CryptoCrazeprivkey> [label]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <CryptoCrazeaddress>\n"
            "Reveals the private key corresponding to <CryptoCrazeaddress>.");
...
Value bip38encrypt(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"CRZ_address\"\n"
                "\nEncrypts a private key corresponding to 'CRZ_address'.\n"
                "\nArguments:\n"
                "1. \"CRZ_address\"   (string, required) The CRZ address for the private key (you must hold the key already)\n"
    ...
    CBitcoinAddress address;
    if (!address.SetString(strAddress))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid CRZ address");
    CKeyID keyID;
    if (!address.GetKeyID(keyID))
        throw JSONRPCError(RPC_TYPE_ERROR, "Address does not refer to a key");
    CKey vchSecret;
